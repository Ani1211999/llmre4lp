...
class GatedGCN(nn.Module):
    def __init__(self, in_feats, hidden_dim, out_dim, n_steps=3, dropout=0.5):
        ...
        self.gnn = GatedGraphConv(
            ...
            n_steps=n_steps,
            n_etypes=1  # 👈 required for homogeneous graphs
        )
    ...
    def forward(self, g, x):
        ...
        # Use edge type 0 for all edges (homogeneous)
...
#     parser.add_argument('--wandb_name', type=str, default='gatedgcn-run')
...
#     # Convert train edges to a DGL graph for message passing
...
#     #dataset_name = os.path.basename(args.npz_path).replace('.npz', '')
#     result_file = os.path.join(result_dir, f"{args.dataset_name}_gatedgcn_results.txt")
