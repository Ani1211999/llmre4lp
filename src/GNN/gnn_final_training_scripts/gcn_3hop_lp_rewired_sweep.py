...
def run(config=None):
    with wandb.init(config=config):
        ...
        rewired_edges = np.load(config.rewired_edges_path)
        ...
        # Combine train and LLM-predicted edges
        rewired_edges = torch.tensor(rewired_edges, dtype=torch.long).to(device)
