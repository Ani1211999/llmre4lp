...
def main():
    ...
    parser.add_argument('--wandb_project', type=str, default='link-prediction-3hop-gnn-rewired')
    parser.add_argument('--wandb_name', type=str, default='run')
    parser.add_argument('--rewired_edges_path', type=str, required=True, help="Path to the llm predicted long-range edges to be added to the graph")
    ...
    llm_pred_rewired_edges = torch.tensor(np.load(args.rewired_edges_path), dtype=torch.long).to(device)
    #llm_pred_rewired_edges = to_undirected(llm_pred_rewired_edges)

    edge_index = torch.cat([train_edges, llm_pred_rewired_edges], dim=1)

    edge_index = to_undirected(edge_index).to(device)  # Use 1-hop train edges for message passing
