...
def run(config=None):
    with wandb.init(config=config):
        ...
        # Make train edges undirected for message passing
        train_edges = torch.cat([train_edges, train_edges[[1, 0]]], dim=1)
        g = dgl.graph((train_edges[0], train_edges[1]), num_nodes=x.size(0)).to(device)
        ...
        # result_file = os.path.join(result_dir, f"{dataset_name}_1hop_gatedgcn_lr{config.lr}_hd{config.hidden_dim}_do{config.dropout}_seed{config.seed}.txt")
