...
def run(config=None):
    with wandb.init(config=config):
        ...
        # result_dir = f"results_rewired_1hop_spectral/{config.dataset_name}"
        # os.makedirs(result_dir, exist_ok=True)
        # result_file = os.path.join(
        #     result_dir,
        #     f"{config.dataset_name}_1hop_gatedgcn_gamma{config.gamma}_lr{config.lr}_hd{config.hidden_dim}_do{config.dropout}_seed{config.seed}.txt"
        # )
        # with open(result_file, "w") as f:
        #     f.write(f"Best Test AUC: {best_test_auc:.4f}\n")
        #     f.write(f"Corresponding Test AUC: {best_test_auc:.4f}\n")
        #     f.write(f"Best Epoch: {best_epoch}\n")
        #     f.write(f"LR: {config.lr}, Dropout: {config.dropout}, Seed: {config.seed}, Gamma: {config.gamma}\n")
