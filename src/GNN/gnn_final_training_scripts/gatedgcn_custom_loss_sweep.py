...
def run(config=None):
    with wandb.init(config=config):
        ...
        result_file = os.path.join(result_dir, f"{config.dataset_name}_gamma{config.gamma}_lr{config.lr}_hd{config.hidden_dim}_do{config.dropout}_seed{config.seed}.txt")
