...
def spectral_regularization_loss(graph, gamma=0.05):
    """Compute -γ * λ₂(Laplacian) as spectral regularization."""

    # Move graph to CPU and get edges
    g_cpu = graph.cpu()
    src, dst = g_cpu.edges()
    ...
    adj = sp.coo_matrix(
        (np.ones(len(src)), (src.numpy(), dst.numpy())),
    ...
    # Compute λ₂ (second smallest eigenvalue)
    try:
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        spectral_loss = spectral_regularization_loss(g, gamma=0.05)
