...
class GatedGCN(nn.Module):
    def __init__(self, in_feats, hidden_size, num_layers, dropout):
        ...
        # Linear layer to reduce input dimension from in_feats to hidden_size
        ...
        for _ in range(num_layers - 1):
            self.layers.append(GatedGraphConv(hidden_size, hidden_size, n_steps=1, n_etypes=1))
        # Output layer: hidden_size * 2 (concatenated edge embeddings) -> 1
    ...
    def forward(self, g, features):
        ...
        h = F.relu(h)  # Apply ReLU activation after projection
...
def train_model(model, g, features, train_edges, train_neg_edges, val_edges, val_neg_edges, test_edges, test_neg_edges, epochs, lr, patience):
    ...
    # Total number of edges for indexing
    total_edges = (
        len(train_edges[0]) + len(train_neg_edges[0]) +
        len(val_edges[0]) + len(val_neg_edges[0]) +
        len(test_edges[0]) + len(test_neg_edges[0])
    )

    for epoch in range(epochs):
        model.train()
        optimizer.zero_grad()
        # Forward pass for all edges
        scores = model(g, features)
        # Extract scores for training edges
        ...
        # Apply class weighting for imbalance (16.80% Yes, 83.20% No)
        pos_weight = torch.tensor([0.832 / 0.168]).to(scores.device)  # Inverse ratio of "No" to "Yes"
        ...
        model.eval()
        with torch.no_grad():
            # Extract scores for validation edges
...
def main():
    ...
    parser.add_argument('--rewired_edges', required=True)
    ...
    # Combine all edges
    rewired_edges = np.load(args.rewired_edges)
    all_edges = np.concatenate([edges, rewired_edges], axis=1)
    all_edges = np.concatenate([all_edges, train_neg_edges, val_edges, val_neg_edges, test_edges, test_neg_edges], axis=1)
